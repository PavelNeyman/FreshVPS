...
module_blocky_install() {
  ...
  arch="$(arch_go)"
  # Map to Blocky release asset names
  local asset_arch
  case "${arch}" in
  ...
  # Assets like blocky_v0.35.0_Linux_x86_64.tar.gz
...
EOF
  ...
  # Disable systemd-resolved stub if present (frees port 53)
...
EOF
    ...
    # Point resolv.conf to blocky later; temporary public DNS for bootstrap
